...
function getErrorMessage(status, defaultMessage) {
  if (status === 400) {
    // For 400 errors, try to extract the specific error message
    if (defaultMessage && typeof defaultMessage === 'string') {
      // If it's a validation error, show it directly
      if (defaultMessage.includes('Invalid or missing') || defaultMessage.includes('Missing required')) {
  ...
  if (status === 503) {
    return "Service temporarily unavailable. The server is overloaded. Please try again in a moment.";
  }
  if (status >= 500) {
 ...
 * @param {string} params.client_phone - Client phone number
...
export async function sendChatMessage({ 
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (res.ok) {
        // Debug logging
        console.log('[API] Response from /api/ingest:', {
          ok: data?.ok,
          has_replyText: !!data?.replyText,
          replyText: data?.replyText?.substring(0, 100) || null,
        ...
        trackTelemetry('message_send_success', { 
          ...
          replyText_length: data?.replyText?.length || 0,
        });
        return {
          ok: true,
          ...
          conversation_id: data?.conversation_id,
          message_id: data?.message_id,
          handoff_to_human: data?.handoff_to_human || false,
          data,
        };
      }
      ...
      console.error('[API] Request failed:', {
        ...
        attempt: attempt + 1,
        response_preview: typeof data === 'object' ? JSON.stringify(data).substring(0, 200) : String(data).substring(0, 200),
      });
